...
class AwaleGame:
    def __init__(self, player1_agent: Agent, player2_agent: Agent, game_id=None):
        self.board = np.full((16, 2), 2, dtype=np.int8)  # 16 trous, 2 couleurs
        self.scores = np.zeros(2, dtype=np.int32)
        player1_holes = np.array([0, 2, 4, 6, 8, 10, 12, 14], dtype=np.int32)
        player2_holes = np.array([1, 3, 5, 7, 9, 11, 13, 15], dtype=np.int32)
    ...
    def claude_evaluate_V1(self) -> int:
        return numba_type_shit.fast_claude_evaluate_V1(
            self.board,
            self.scores,
            self.current_player,
            self.player_holes
    ...
    def clone(self):
        ...
        cloned_game.moves_log = self.moves_log.copy()
        return cloned_game

    def game_over(self):
        total_seeds = self.board.sum()
        if total_seeds < 8:
            return True
